...
<template>
  <div class="space-y-1 p-1 text-sm bg-white">
    <template v-for="(item, i) in props.items">
      <div v-if="isDividerItem(item)" class="-mx-1 my-1 h-px bg-border" :key="item.type + i"></div>
      <div v-else-if="isGroupItem(item)" class="-mx-1 my-1 border-b border-t border-input p-1" :key="item.label">
        <slot name="group" :item="item">
          <div class="px-2 py-1 font-medium text-normal">{{ item.label }}</div>
        </slot>
